...
string ipc_method = "f"; //fifo for default
int nchannels = 0;
...
void process_newchannel_request (RequestChannel* _channel) { //modify
	nchannels++;
	...
	RequestChannel* data_channel;//new FIFORequestChannel(new_channel_name, FIFORequestChannel::SERVER_SIDE);
	if (ipc_method == "f"){
...
int main (int argc, char* argv[]) {
	...
	//modify
	RequestChannel* control_channel; //new FIFORequestChannel("control", FIFORequestChannel::SERVER_SIDE);
	if (ipc_method == "f"){
